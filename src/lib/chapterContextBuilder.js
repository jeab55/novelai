...
export const SHARED_CRAFT_RULES = `[กฎพื้นฐานงานเขียนออนไลน์ (ยึดทุกตอน — เป็นเพียงวิธีนำเสนอ ไม่ใช่สไตล์ ห้ามกลบสไตล์เฉพาะตัวของนักเขียน)]
- เปิดตอนให้ติดหมัดใน 1-2 ย่อหน้าแรก มีปม คำถาม หรือสิ่งผิดปกติทันที ไม่เปิดด้วยบรรยายฉากยาวหรือเล่าอดีต
- ปิดทุกตอนด้วยเบ็ด (คำถามค้าง / ข้อมูลใหม่กระแทก / การตัดสินใจชวนลุ้น) ยกเว้นตอนจบเรื่อง
- บทสนทนาขับเรื่อง สัดส่วนราว 40-60% ของฉากปกติ (ปรับเพิ่มได้ตามแนวที่นักเขียนถนัด)
- กฎบทสนทนา: ขึ้นบรรทัดใหม่ทุกครั้งที่เปลี่ยนผู้พูด / ใช้การกระทำ-ท่าทาง (action beat) ติดกับคำพูดเพื่อระบุผู้พูดและอารมณ์ แทนการเขียน "เขากล่าว" ซ้ำๆ / ห้ามใช้รูปแบบ "ชื่อ: คำพูด"
    ...
    if (writtenBefore.length > 0) {
      ...
      if (writtenBefore.length > 2) {
        ...
        writtenBefore.slice(0, -2).forEach((ch) => {
          ctx += `• ตอนที่ ${ch.order} "${ch.title}": ${summarizeContent(ch.content, 400)}\n`;
        });
      }
