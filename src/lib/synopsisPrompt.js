...
// แนวทางคำโปรยที่ดี (เป็นหลักการ ไม่ใช่สูตรตายตัว)
const BLURB_RULES = `[แนวทางคำโปรยที่ดี — ยึดเป็นหลักการ ไม่ใช่สูตรสำเร็จ]
- กระชับ ดึงดูดให้อยากอ่านต่อทันที
- สื่อแนวและอารมณ์ของเรื่องให้ชัดตามแนวจริง
- มีปม/เดิมพันที่น่าติดตาม แต่ห้ามสปอยตอนจบ
- เขียนด้วยภาษาเฉพาะของเรื่องนี้ ห้ามใช้วลีแม่แบบหรือโครงสร้างซ้ำเดิมทุกเรื่อง
- คำโปรยแต่ละแบบต้องเปิดเรื่อง/วางมุม/ลีลาให้ต่างกันชัดเจน ไม่ใช่แค่สลับคำ`;
...
export function buildBlurbPrompt(story = {}, characters = [], options = {}) {
  const { variants = 3, writerSystemPrompt = "", marketMode = false, sourceMode = "anchored" } = options;
  ...
  const tropeBlock = tropes
    ...
    : "";
  const variety = varietyInstruction({ count: Math.min(variants, 3) });
  const sourceBlock = sourceMode === "free"
    ? `\n[โหมดแตกแนวใหม่อิสระ]\nไม่ต้องยึดติดกับเรื่องย่อหรือพล็อตเดิม — ลองวางมุมและทิศทางใหม่ที่สดกว่าเดิม โดยยังคงชื่อเรื่อง แนว และยุคสมัยไว้\n`
    : "";

  return `${writerCtx}คุณคือนักเขียนนิยายไทยมือทอง เชี่ยวชาญการเขียน "คำโปรย" ที่ดึงดูดให้คนกดอ่าน

[ข้อมูลจริงของเรื่องนี้ — ใช้เป็นแกนในการเขียน ห้ามแต่งลอยๆ ทั่วไป]
${buildStoryContext(story, characters)}
${tropeBlock}${sourceBlock}
${BLURB_RULES}

${variety}

[งานที่ต้องทำ]
เขียนคำโปรย ${variants} แบบที่ "แตกต่างกันชัดเจน" ทั้งมุมเปิด ลีลา และอารมณ์ ทุกแบบต้องสะท้อนเนื้อเรื่องจริงจากข้อมูลด้านบน (แนว, เรื่องย่อ, โครงเรื่อง, ยุค/ฉาก, ตัวละครและปม)
