...
function buildOneShotSection(novel) {
  ...
  ctx += `※ ข้อ 2-8 ด้านล่างเป็นค่าเริ่มต้นของเรื่องสั้นทั่วไป — ถ้าสไตล์นักเขียนประจำเรื่องกำหนดต่าง (เช่น slice-of-life, หลายตัวละคร, เล่ากรอบซ้อน, เปิดเรื่องแบบค่อยเป็นค่อยไป) ให้ยึดสไตล์นักเขียนก่อนเสมอ ห้ามให้ข้อเหล่านี้กลบลายเซ็นนักเขียน; คงไว้เสมอเฉพาะ "จบสมบูรณ์ในตอนเดียว" และรูปแบบตอนจบที่ผู้เขียนเลือก\n`;
  ctx += `1. เขียนเรื่องสั้นสมบูรณ์ในตัวเอง — มีเปิดเรื่อง ปมกลางเรื่อง และจุดพีคตอนจบ\n`;
  ctx += `2. โดยทั่วไปเน้นแก่นเดียว อารมณ์หลักเดียว ให้ทุกฉากรับใช้แก่นหลัก (ปรับได้ตามสไตล์นักเขียน)\n`;
  ctx += `3. โดยทั่วไปโฟกัสตัวละครหลักไม่กี่คนเพื่อให้ลึก (ปรับจำนวนได้ตามสไตล์/เรื่อง ไม่จำกัดตายตัว)\n`;
  ctx += `4. โดยทั่วไปเปิดเรื่องกระชับ เข้าเรื่องไว (เว้นแต่สไตล์นักเขียนกำหนดวิธีเปิดแบบอื่น)\n`;
  ctx += `5. จัดจังหวะให้ครบทั้งเปิดปม–บีบให้ตึง–คลายด้วยจุดพีค ภายในตอนเดียว (สัดส่วนปรับได้ตามเรื่อง ไม่ต้องยึดเปอร์เซ็นต์ตายตัว)\n`;
  ...
  ctx += `8. ใช้ภาษาตามสไตล์นักเขียนประจำเรื่อง (ถ้าไม่ระบุ ให้กระชับ สละสลวย เห็นภาพ)\n\n`;
  return ctx;
}
...
function buildSystemPrompt(novel, characters, worldEntries, plotEvents, allChapters, currentOrder, linkedEvent, writerPrompt, previousSeasonLastChapter) {
  ...
  if (isOneShot) {
    ...
    ctx += `- ให้เนื้อหาลึกและต่อเนื่องสอดคล้องกับโครงเรื่องภาพรวม ตัวละคร และตอนก่อนหน้า\n`;
